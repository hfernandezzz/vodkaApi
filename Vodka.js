...
function checkdrink(name) {
    for (const vodka of vodkaRequest.drinks) {
        if (vodka.strDrink.toLowerCase() == name.toLowerCase() || vodka.idDrink == name) {
            return true
        }
    }
    return false
...
function getDrink(name) {
    for (const vodka of vodkaRequest.drinks) {
        if (vodka.strDrink.toLowerCase() == name.toLowerCase() || vodka.idDrink == name) {
            return vodka //En este caso le diremos vodka al return para que devuelva el objeto vodka.
        }
    }
}
...
//Lista de bebidas
console.log("Carta de vodkas: \n") // \n se utiliza para los saltos de línea.
for (const vodka of vodkaRequest.drinks) {
    console.log("ID: " + vodka.idDrink + " | Nombre: " + vodka.strDrink + "\n")
}

//Queremos que devuelva falso solo cuando termine la lista por lo que hay que poner el return false fuera del bucle for. 
//Si no lo realizaría únicamente con el primer objeto ya que solo puede devolver una vez(el return).

//Verificación de la bebida que introduzca el usuario.
let inputUser = sc("¿Qué vodka deseas?: ")
while (checkdrink(inputUser) == false) {
    inputUser = sc("Este vodka no está en la carta o has introducido un dato erroneo. Porfavor introduzca el 'ID' o el 'Nombre' del vodka que desea: ")
}
console.log(" ")

//Muestra los ingredientes de la bebida y sus medidas.
const drink = getDrink(inputUser)
console.log("ID: " + getDrink(inputUser).idDrink + " | Nombre: " + getDrink(inputUser).strDrink + "\n")
for (let i = 1; i <= 15; i++) {
    const ingredient = drink['strIngredient' + i] //Dentro de la bebida itera cada ingrediente del 1 al 15 en este caso. Al tener las medidas la misma enumeración se meten dentro del mismo for.
    const measure = drink['strMeasure' + i] != null ? drink['strMeasure' + i].trim() : "1 Unit" //Condición ternaria != condición ? se ejecuta si es verdadero : se ejecuta si es falso.
    if (ingredient != null) {
        console.log("Ingrediente " + i + ": " + ingredient + " (" + measure + ") \n")
    }
}

//La función trim() quita los espacios del inicio y final de una string.
//Alt + flechitas para mover la linea arriba y abajo.